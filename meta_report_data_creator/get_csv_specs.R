...
get_csv_specs <- function(files, expected_colnames = c("Investor.Name", "Portfolio.Name", "ISIN", "MarketValue", "Currency")) {
  ...
  files_df$file_encoding <- pacta.portfolio.analysis::guess_file_encoding(files_df$filepath)
  ...
  files_df$delimiter <- pacta.portfolio.analysis::guess_delimiter(files_df$filepath)
  ...
  files_df$decimal_mark <- pacta.portfolio.analysis:::guess_decimal_mark(files_df$filepath)
  ...
  files_df$grouping_mark <- pacta.portfolio.analysis:::guess_grouping_mark(filepaths = files_df$filepath)
...
has_newline_at_end <- function(filepaths) {
  filepaths <- pacta.portfolio.analysis:::simplify_if_one_col_df(filepaths)
  stopifnot("`filepaths` must be a character vector" = typeof(filepaths) == "character")
  filepaths <- pacta.portfolio.analysis:::canonize_path(filepaths)
